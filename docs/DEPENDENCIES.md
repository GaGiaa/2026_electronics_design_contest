...
`stm32h723_app/` 的新增运行时硬件依赖为 BNO055 九轴智能传感器模块。模块使用原生 UART 协议，配置为 `115200 bit/s`、8-N-1；H723 `PB6/USART1_TX` 接模块 RX，模块 TX 接 `PB7/USART1_RX`，并与板端共地。模块 UART IO 必须兼容 3.3 V，且必须选择 BNO055 原生 UART 模式。本工程不引入第三方 BNO055 软件库，寄存器协议实现保留在仓库内的纯 C `App/app_bno055`。
