...
线控要求 `sequence` 非零、ADC 超时掩码为 0 且 `line_strength` 达到进入阈值 10000；已有效
后使用退出阈值 5000。丢线时冻结 PID 并保持上次转向输出 100 ms，之后清零目标并复位。
...
灰度有效进入/退出阈值调整为 `10000/5000`。新增主机单元测试覆盖默认参数的有效判定、转向
...
- 灰度白黑归一化、位图、横向误差、全白、全黑和丢线状态；当前 `line_strength=10000`
  的进入阈值要求黑线通常覆盖约三个以上通道，窄线覆盖范围仍需硬件验证；
