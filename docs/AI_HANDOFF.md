...
## H723 Task 4 Test Alignment (2026-08-01)

- Updated `tests/test_stm32h723_task4.c` to exercise the current 300 mm/s,
  450 mm distance-braking configuration. The scenario now reaches 1100 mm,
  leaving 400 mm before the 1500 mm target.
- Updated `tests/test_stm32h723_task4_integration.ps1` so distance macros are
  validated in `app_config.h`, where they are defined, rather than requiring
  them to appear in `app_task4.c`.
- Validation passed: `tests/test_stm32h723_task4.ps1`,
  `tests/test_stm32h723_task4_integration.ps1`, and `git diff --check`.
