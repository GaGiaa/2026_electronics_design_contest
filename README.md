...
`stm32h723_app` 已接入任务 2 至任务 6 的按键菜单、灰度巡线、钢珠平衡和 OLED 运行页。任务 2 以
`360 mm/s` 循迹并在 `black_count >= 5` 时停车；任务 4 采用 `350 mm/s`、`100 mm/s^2`、
10 s 速度曲线；任务 5/6 采用 `220 mm/s`、`100 mm/s^2`、30 s 曲线。任务 3 使用 K230
钢珠位置闭环，按 B1 执行 `125 -> 225 -> 75 mm` 流程并在 OLED 上计时。

任务菜单末页为 `BALL SET`：将钢珠放在新的平衡位置后按 B1，系统读取有效视觉位置并保存为通用目标。
任务模式始终保持 ID 3 钢珠闭环；遥控器模式只有 SB 中档允许 ID 3 闭环，SB 低档及其他档位输出零电流。

ID 3 平衡机构仍保留 JY901S 倾角闭环与 `70–210 deg` 正常活动范围；新增 K230 钢珠位置反馈，
位置 PID 以 40 Hz 生成水管倾角目标，并在上电首次归零后要求 OLED pitch 零位校准。底盘和巡线 VOFA 实现
已接入 UART8 互斥保护，但底盘 VOFA 默认关闭；启用前需确认没有其他 UART8 遥测模式。
