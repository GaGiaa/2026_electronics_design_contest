...
#define BOARD_BLUETOOTH_UART_TX_QUEUE_LENGTH 8U

#if APP_BLUETOOTH_UART_ENABLE
typedef struct {
    uint16_t length;
    uint8_t data[BOARD_BLUETOOTH_UART_TX_MESSAGE_MAX_LENGTH];
} board_bluetooth_uart_tx_message_t;
...
static uint8_t g_tx_queue_storage[
    BOARD_BLUETOOTH_UART_TX_QUEUE_LENGTH *
    sizeof(board_bluetooth_uart_tx_message_t)];
#endif

void board_bluetooth_uart_disable(void)
{
    DL_UART_Main_disableInterrupt(UART_2_INST,
...
{
    g_rx_queue = queue;
    g_tx_queue = xQueueCreateStatic(BOARD_BLUETOOTH_UART_TX_QUEUE_LENGTH,
                                    sizeof(board_bluetooth_uart_tx_message_t),
                                    g_tx_queue_storage, &g_tx_queue_buffer);
    configASSERT(g_tx_queue != NULL);
    NVIC_SetPriority(UART_2_INST_INT_IRQN,
...
void board_bluetooth_uart_irq_handler(void)
{
    ...
    if (DL_UART_Main_getPendingInterrupt(UART_2_INST) == DL_UART_MAIN_IIDX_RX) {
        while (!DL_UART_Main_isRXFIFOEmpty(UART_2_INST)) {
            ...
            if ((g_rx_queue == NULL) ||
                (xQueueSendFromISR(g_rx_queue, &byte,
...
{
    return DL_UART_Main_isTXFIFOFull(UART_2_INST);
}
...
{
    board_bluetooth_uart_tx_message_t message;
    size_t index;

    if ((data == NULL) || (length == 0U) ||
        (length > BOARD_BLUETOOTH_UART_TX_MESSAGE_MAX_LENGTH) ||
        (g_tx_queue == NULL)) {
        return;
    }
    ...
    for (;;) {
        if (xQueueReceive(g_tx_queue, &message, portMAX_DELAY) == pdPASS) {
            for (index = 0U; index < message.length; ++index) {
                while (board_bluetooth_uart_tx_full()) {
                    taskYIELD();
                }
                board_bluetooth_uart_transmit(message.data[index]);
            }
        }
    }
}
