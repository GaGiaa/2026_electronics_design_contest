...
$config = Join-Path $projectDir 'app_config.h'
$main = Join-Path $projectDir 'main.c'
$oledHeader = Join-Path $projectDir 'board_oled.h'
$oledSource = Join-Path $projectDir 'board_oled.c'
$font = Join-Path $projectDir 'board_oled_font.c'
...
Assert-Contains -Path $main -Pattern '#include "board_oled\.h"' -Description 'Application must include the OLED driver'
