...
Assert-Contains -Path $projectFile -Pattern '\.\.\\\.\.\\mspm0g3507_app\\algorithms\\course_following\\course_following\.c' -Description 'Keil project must compile the course following layer'
Assert-Contains -Path $projectFile -Pattern '\.\.\\\.\.\\mspm0g3507_app\\protocols\\vofa\\course_following_telemetry\.c' -Description 'Keil project must compile course following telemetry'
...
Assert-Contains -Path $projectFile -Pattern '\.\.\\\.\.\\mspm0g3507_app\\algorithms\\imu_fusion\\imu_fusion\.c' -Description 'shared IMU fusion source'
...
Assert-Contains -Path (Join-Path $ProjectRoot 'keil\mspm0g3507_app\startup_mspm0g350x_uvision.s') -Pattern '0x00000400' -Description 'Keil system stack must reserve 1 KiB for nested exception paths'
...
Assert-Contains -Path $buildScript -Pattern 'ButtonVofaTelemetryEnable' -Description 'Keil build must support an explicit button VOFA telemetry mode'
...
Assert-Contains -Path $buildScript -Pattern 'LineControlVofaTelemetryEnable' -Description 'Keil build must support an explicit line control VOFA telemetry mode'
Assert-Contains -Path $buildScript -Pattern 'LineControlVofaTelemetryIntervalMs' -Description 'Keil build must support a line control VOFA interval override'
Assert-Contains -Path $buildScript -Pattern 'CourseFollowingVofaTelemetryEnable' -Description 'Keil build must support a course following VOFA override'
Assert-Contains -Path $buildScript -Pattern 'CourseFollowingVofaTelemetryIntervalMs' -Description 'Keil build must support a course following VOFA interval override'
...
Assert-Contains -Path $buildScript -Pattern 'APP_VOFA_SPEED_PID_TELEMETRY_ENABLE=' -Description 'Keil build must inject the speed VOFA compiler define'
Assert-Contains -Path $buildScript -Pattern 'APP_BUTTON_VOFA_TELEMETRY_ENABLE=' -Description 'Keil build must inject the button VOFA compiler define'
Assert-Contains -Path $buildScript -Pattern 'APP_GRAY_VOFA_TELEMETRY_ENABLE=' -Description 'Keil build must inject the gray VOFA compiler define'
Assert-Contains -Path $buildScript -Pattern 'APP_LINE_CONTROL_VOFA_TELEMETRY_ENABLE=' -Description 'Keil build must inject the line control VOFA compiler define'
Assert-Contains -Path $buildScript -Pattern 'APP_COURSE_FOLLOWING_VOFA_TELEMETRY_ENABLE=' -Description 'Keil build must inject the course following VOFA compiler define'
Assert-Contains -Path $buildScript -Pattern 'APP_SERVO_FEATURE_ENABLE=' -Description 'Keil build must inject the servo feature define'
Assert-Contains -Path $readme -Pattern 'APP_GRAY_VOFA_TELEMETRY_ENABLE' -Description 'gray VOFA telemetry documentation'
