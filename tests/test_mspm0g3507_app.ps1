...
foreach ($path in @($projectDir, $main, $ws2812, $ws2812Header, $buzzer, $buzzerHeader, $bmi160, $bmi160Header, $buttons, $buttonsHeader, $grayscale, $grayscaleHeader, $uart, $motor, $motorHeader, $encoder, $encoderHeader, $syscfg, $rtosConfig, $ccsBuildConfig,
...
Assert-Contains -Path $main -Pattern 'GRAY_TELEMETRY_ENABLE\s+[01]U' -Description 'Gray telemetry must use a binary compile-time switch'
Assert-Contains -Path $main -Pattern '#if GRAY_TELEMETRY_ENABLE' -Description 'Gray UART output must be compile-time gated'
Assert-Contains -Path $main -Pattern 'GRAY_SAMPLE_INTERVAL_MS\s+10U' -Description 'Gray task must sample every 10 ms'
Assert-Contains -Path $main -Pattern 'GRAY_TELEMETRY_INTERVAL_MS\s+100U' -Description 'Gray telemetry must run at 10 Hz'
...
Assert-Contains -Path $main -Pattern 'gray,raw=' -Description 'Gray telemetry must report raw channels'
Assert-Contains -Path $main -Pattern 'norm=' -Description 'Gray telemetry must report normalized channels'
Assert-Contains -Path $main -Pattern 'digital=0x' -Description 'Gray telemetry must report digital channels'
