...
foreach ($path in @($projectDir, $main, $ws2812, $ws2812Header, $buzzer, $buzzerHeader, $servo, $servoHeader, $servoMath, $appConfig, $bmi160, $bmi160Header, $imuYaw, $imuYawHeader, $buttons, $buttonsHeader, $grayscale, $grayscaleHeader, $lineTracking, $lineTrackingHeader, $uart, $crsfUart, $crsfProtocol, $crsfProtocolHeader, $crsfControl, $crsfControlHeader, $crsfConfig, $motor, $motorHeader, $encoder, $encoderHeader, $motorControl, $motorControlHeader, $motorPid, $motorPidHeader, $vofaJustFloat, $vofaJustFloatHeader, $syscfg, $rtosConfig, $ccsBuildConfig,
...
Assert-Contains -Path $appConfig -Pattern 'SERVO_FEATURE_ENABLE\s+0U' -Description 'Servo task must default to disabled'
Assert-Contains -Path $appConfig -Pattern 'SERVO_MAX_ANGLE_DEG\s+180U' -Description 'Servo angle range must default to 180 degrees'
Assert-Contains -Path $appConfig -Pattern 'SERVO_MIN_PULSE_US\s+500U' -Description 'Servo minimum pulse must default to 500 us'
Assert-Contains -Path $appConfig -Pattern 'SERVO_MAX_PULSE_US\s+2500U' -Description 'Servo maximum pulse must default to 2500 us'
Assert-Contains -Path $main -Pattern '#if SERVO_FEATURE_ENABLE' -Description 'Servo debug task must be compile-time gated'
...
Assert-Contains -Path $buildScript -Pattern 'SERVO_FEATURE_ENABLE=' -Description 'Build must pass the servo feature switch to the compiler'
