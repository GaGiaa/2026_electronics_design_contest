...
foreach ($path in @($projectDir, $main, $ws2812, $ws2812Header, $buzzer, $buzzerHeader, $bmi160, $bmi160Header, $uart, $motor, $motorHeader, $encoder, $encoderHeader, $motorControl, $motorControlHeader, $motorPid, $motorPidHeader, $vofaJustFloat, $vofaJustFloatHeader, $syscfg, $rtosConfig, $ccsBuildConfig,
...
Assert-Contains -Path $main -Pattern '#define VOFA_SPEED_PID_TELEMETRY_ENABLE [01]U' -Description 'VOFA speed PID telemetry must define a default mode'
...
Assert-Contains -Path $main -Pattern '#if !VOFA_SPEED_PID_TELEMETRY_ENABLE' -Description 'VOFA mode must compile out the UART echo task'
...
Assert-Contains -Path $main -Pattern '#if IMU_TELEMETRY_ENABLE && !VOFA_SPEED_PID_TELEMETRY_ENABLE' -Description 'VOFA mode must suppress IMU UART text'
