...
static void make_frame(uint8_t valid, float distance_mm, uint8_t frame[APP_K230_FRAME_SIZE])
{
    uint16_t crc;
    ...
    frame[2] = valid;
    put_float_le(&frame[3], distance_mm);
...
static void test_publishes_little_endian_signed_distance(void)
{
    ...
    assert(sample.valid);
    assert(fabsf(sample.distance_mm + 12.5f) < 0.0001f);
    assert(sample.last_frame_ms == 123U);
    assert(sample.valid_frame_count == 1U);
}

static void test_publishes_invalid_frame_as_zero_distance(void)
{
    app_k230_parser_t parser;
    app_k230_sample_t sample = {0};
    uint8_t frame[APP_K230_FRAME_SIZE];
    ...
    assert(!sample.valid);
    assert(sample.distance_mm == 0.0f);
...
static void test_rejects_bad_crc_and_resynchronizes_after_noise(void)
{
    ...
    assert(sample.valid);
    assert(fabsf(sample.distance_mm - 3.25f) < 0.0001f);
}
...
static void test_rejects_invalid_valid_field_without_overwriting_sample(void)
{
    ...
    assert(sample.valid);
    assert(fabsf(sample.distance_mm - 8.0f) < 0.0001f);
    assert(sample.valid_frame_count == 1U);
}

int main(void)
{
    test_publishes_little_endian_signed_distance();
    test_publishes_invalid_frame_as_zero_distance();
