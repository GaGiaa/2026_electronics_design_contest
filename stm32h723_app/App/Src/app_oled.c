...
{
    static const char *const names[] = {
        "IDLE", "WAIT CONF", "MOVE 128", "MOVE 151", "FINISHED", "FAULT"
    };
    return phase < (sizeof(names) / sizeof(names[0])) ? names[phase] : "UNKNOWN";
}
...
static board_oled_status_t render_runtime_page(void)
{
    ...
    const bool show_task_page = show_task2 || show_task3 || show_task4 || show_task56;
    ...
    const uint32_t task_elapsed_ms = show_task3 ? g_h723_debug.task3.elapsed_ms :
        (show_task56 ?
